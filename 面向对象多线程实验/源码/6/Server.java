...
public class Server{
   private static DataOutputStream output; //初始化输入输出流
   ...
   public void runServer() throws SQLException{//运行服务端
      try{
         server = new ServerSocket(12345,100);//初始化服务端
         while ( true ){
            try{
               waitForConnection(); 
               getStreams(); 
   ...
   private void waitForConnection() throws IOException{//等待客户端，进行反应连接
   ...
   private void processConnection() throws IOException, SQLException{//链接过程显示，包含各种方法
